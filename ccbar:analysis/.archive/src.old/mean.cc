...
int main(int argc, char* argv[]) {
  ...
  char binfname[2048];
  addSuffix(ofname, "bin", binfname);

  if (isJackknife) {
    jackknifeMean(argv, binfname, arrayLength, fileCountTotal);
  } else if (isJackknifeD) {
    jackknifeMeanD(argv, binfname, arrayLength, fileCountTotal);
  } else {
    arithmeticMean(argv, binfname, arrayLength, fileCountTotal);
  }

  if (isSaveTxt) {
    char txtfname[2048];
    addSuffix(ofname, "txt", txtfname);
    bin2txt(binfname, txtfname, arrayLength);
  }
